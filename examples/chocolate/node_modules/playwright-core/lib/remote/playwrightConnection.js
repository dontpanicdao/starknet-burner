...
class PlaywrightConnection {
  constructor(lock, mode, ws, isDebugControllerClient, options, preLaunched, log, onClose) {
    this._ws = void 0;
  ...
  _initDebugControllerMode() {
    ...
    this._cleanups.push(() => (0, _processLauncher.gracefullyCloseAll)()); // Always create new instance based on the reused Playwright instance.
  ...
  async _initReuseBrowsersMode(scope) {
    ...
    if (!browser) {
      browser = await playwright[this._options.browserName || 'chromium'].launch((0, _instrumentation.serverSideCallMetadata)(), { ...this._options.launchOptions,
        headless: false
      });
      browser.on(_browser.Browser.Events.Disconnected, () => {
        // Underlying browser did close for some reason - force disconnect the client.
