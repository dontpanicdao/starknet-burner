...
class PlaywrightServer {
  ...
  async listen(port = 0) {
    ...
    this._wsServer.on('connection', (ws, request) => {
      ...
      const semaphore = isDebugControllerClient ? controllerSemaphore : browserSemaphore; // If we started in the legacy reuse-browser mode, create this._preLaunchedPlaywright.
      // If we get a reuse-controller request,  create this._preLaunchedPlaywright.

      if (isDebugControllerClient || this._mode === 'reuse-browser' && !this._preLaunchedPlaywright) this.preLaunchedPlaywright(); // If we have a playwright to reuse, consult controller for reuse mode.

      let mode = this._mode;
      if (mode === 'auto' && (_this$_preLaunchedPla = this._preLaunchedPlaywright) !== null && _this$_preLaunchedPla !== void 0 && _this$_preLaunchedPla.debugController.reuseBrowser()) mode = 'reuse-browser';
      if (mode === 'reuse-browser') semaphore.setMax(1);else semaphore.setMax(this._options.maxConcurrentConnections);
      const connection = new _playwrightConnection.PlaywrightConnection(semaphore.aquire(), mode, ws, isDebugControllerClient, {
        enableSocksProxy,
        browserName,
        launchOptions
      }, {
