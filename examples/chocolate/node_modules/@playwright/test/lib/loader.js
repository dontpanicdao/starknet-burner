...
class Loader {
  ...
  async _processConfigObject(config, configDir) {
    ...
    this._assignUniqueProjectIds(this._fullConfig.projects); // TODO: restore or remove once groups are decided upon.
  ...
  _resolveProject(config, fullConfig, projectConfig, throwawayArtifactsPath) {
    ...
    let screenshotsDir = takeFirst(projectConfig.screenshotsDir, config.screenshotsDir, path.join(testDir, '__screenshots__', process.platform, name));
...
function validateProjectGroups(file, config) {
  var _config$projects;

  if (config.groups === undefined) return;
  const projectNames = new Set((_config$projects = config.projects) === null || _config$projects === void 0 ? void 0 : _config$projects.filter(p => !!p.name).map(p => p.name));

  for (const [groupName, group] of Object.entries(config.groups)) {
    function validateProjectReference(projectName) {
      if (projectName.trim() === '') throw (0, _util.errorWithFile)(file, `config.groups.${groupName} refers to an empty project name`);
      if (!projectNames.has(projectName)) throw (0, _util.errorWithFile)(file, `config.groups.${groupName} refers to an unknown project '${projectName}'`);
    }

    group.forEach((step, stepIndex) => {
      if ((0, _utils.isString)(step)) {
        ...
        step.forEach((item, itemIndex) => {
          ...
          if ((0, _utils.isString)(item)) {
            ...
            if ((0, _utils.isString)(project)) {
              validateProjectReference(project);
            } else if (Array.isArray(project)) {
              project.forEach((name, projectIndex) => {
                if (!(0, _utils.isString)(name)) throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].project[${projectIndex}] contains non string value.`);
            ...
            if ('grep' in item) {
              if (Array.isArray(item.grep)) {
                item.grep.forEach((item, grepIndex) => {
                  if (!(0, _utils.isRegExp)(item)) throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].grep[${grepIndex}] must be a RegExp`);
                });
              } else if (!(0, _utils.isRegExp)(item.grep)) {
                throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].grep must be a RegExp`);
              }
            }
            ...
            if ('grepInvert' in item) {
              if (Array.isArray(item.grepInvert)) {
                item.grepInvert.forEach((item, index) => {
                  if (!(0, _utils.isRegExp)(item)) throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].grepInvert[${index}] must be a RegExp`);
                });
              } else if (!(0, _utils.isRegExp)(item.grepInvert)) {
                throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].grepInvert must be a RegExp`);
              }
            }
            ...
            for (const prop of ['testIgnore', 'testMatch']) {
              if (prop in item) {
                ...
                if (Array.isArray(value)) {
                  value.forEach((item, index) => {
                    if (typeof item !== 'string' && !(0, _utils.isRegExp)(item)) throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].${prop}[${index}] must be a string or a RegExp`);
                  });
                } else if (typeof value !== 'string' && !(0, _utils.isRegExp)(value)) {
                  throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}].${prop} must be a string or a RegExp`);
                }
              }
            }
          } else {
            throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}] unexpected group entry ${JSON.stringify(step, null, 2)}`);
          } // We can relax this later.


          if (parallelProjectNames.has(projectName)) throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}][${itemIndex}] group mentions project '${projectName}' twice in one parallel group`);
          parallelProjectNames.add(projectName);
        });
      } else {
        throw (0, _util.errorWithFile)(file, `config.groups.${groupName}[${stepIndex}] unexpected group entry ${JSON.stringify(step, null, 2)}`);
      }
    });
  }
}
